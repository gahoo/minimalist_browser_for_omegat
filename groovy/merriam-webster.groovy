...
def FILENAME = "merriam-webster.groovy"
def KEY = "merriam_webster"
def TITLE = "Merriam Webster"
def DOMAIN = /^.*/
def URL = "http://192.168.1.100:5678/merriam-webster"
...
Core.getEditor().registerPopupMenuConstructors(1000, new IPopupMenuConstructor() {
    ...
    void addItems(JPopupMenu menu, JTextComponent comp, int mousepos, boolean isInActiveEntry,
        ...
        if (isInActiveEntry) {
            menu.addSeparator()
            JMenuItem menuItem = menu.add("Search in Merriam Webster")
...
KeyStroke keystroke = KeyStroke.getKeyStroke(KeyEvent.VK_SLASH, COMMAND_MASK)
