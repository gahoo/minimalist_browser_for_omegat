...
def FILENAME = "linguee.groovy"
def KEY = "LINGUEE"
def TITLE = "Linuguee"
def DOMAIN = /^.*/
def BASE_URL = "http://192.168.1.100:5678/linguee"

String sourceCode = 'en'
String targetCode = 'zh'
def codeLang = [en: 'english', zh: 'chinese']
...
String sourceLang = codeLang[sourceCode]
String targetLang = codeLang[targetCode]
def URL = "${BASE_URL}/?source_lang=${sourceLang}&target_lang=${targetLang}&query="
...
Core.getEditor().registerPopupMenuConstructors(1000, new IPopupMenuConstructor() {
    ...
    void addItems(JPopupMenu menu, JTextComponent comp, int mousepos, boolean isInActiveEntry,
        ...
        if (isInActiveEntry) {
            menu.addSeparator()
            JMenuItem menuItem = menu.add("Search in Linguee")
...
KeyStroke keystroke = KeyStroke.getKeyStroke(KeyEvent.VK_COMMA, COMMAND_MASK)
def actionMapKey = "searchInLinguee"
...
def projectEventListener = new IProjectEventListener() {
    ...
    void onProjectChanged(IProjectEventListener.PROJECT_CHANGE_TYPE project_change_type) {
        if (project_change_type == IProjectEventListener.PROJECT_CHANGE_TYPE.LOAD ||
            ...
            sourceCode = pp.getSourceLanguage().getLanguageCode().toLowerCase()
            targetCode = pp.getTargetLanguage().getLanguageCode().toLowerCase()
            sourceLang = codeLang[sourceCode]
            targetLang = codeLang[targetCode]
            URL = "${BASE_URL}/?source_lang=${sourceLang}&target_lang=${targetLang}&query="
            println("Linguee URL change to ${URL}")
        }
    }
}
